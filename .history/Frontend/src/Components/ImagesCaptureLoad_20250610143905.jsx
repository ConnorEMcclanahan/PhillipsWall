...
const ImagesCaptureLoad = () => {
    ...
    const processImage = async (imageData) => {
        ...
        try {
            ...
            // Process the image via backend which uses OpenAI Vision
            const processUrl = `${API_BASE}/process-image`;
            console.log("Processing at:", processUrl);
            ...
            const response = await fetch(processUrl, {
                ...
                body: JSON.stringify({ 
                    image: imageData,
                    model: 'gpt-4o' // Specify model for best OCR
                }),
    ...
    const saveToDatabase = async () => {
        ...
        try {
            ...
            // Create a fixed date for Summer 2025 (July)
            const summerDate = new Date(2025, 6, 15).toISOString(); // July 15, 2025
            
            // Format the payload differently to match what the backend expects
            const payload = {
                answer_text: modelResult.answer || "No text detected",
                question_id: parseInt(modelResult.question_id) || 1,
                created_at: summerDate,
                x_axis_value: modelResult.x_axis_value !== null ? modelResult.x_axis_value : Math.random(),
                y_axis_value: modelResult.y_axis_value !== null ? modelResult.y_axis_value : Math.random(),
                answer_language: modelResult.language || "en"
                // Don't include post_it_color for now, as it might be causing issues
            };
            
            console.log("Trying simpler payload:", payload);
            
            // Make a more direct request
            ...
            const response = await fetch(url, {
                ...
                // No credentials option to avoid CORS issues
            });
            ...
            if (!response.ok) {
                ...
                if (response.status === 405) {
                    // Try a different approach - this might be a Flask route issue
                    alert("Server error 405. Your post-it was created but couldn't be saved. Check with administrator.");
            ...
            // Dispatch event for AIQuestionsDisplay to update
            const newPostEvent = new CustomEvent('newPostItAdded');
