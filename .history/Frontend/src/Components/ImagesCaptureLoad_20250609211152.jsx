...
const ImagesCaptureLoad = () => {
    ...
    // Update the processImage function to be less strict with validation
    const processImage = async (imageData) => {
        ...
        try {
            ...
            console.log("Backend response:", result); // Add this to see the actual response
            
            // LESS STRICT VALIDATION - Accept any response with properties
            if (result) {
                // Even if text detection failed, create a default result
                setModelResult({
                    answer: result.answer || result.text || "Text could not be automatically detected",
                    question_id: result.question_id || "1", // Default question ID
                    color: result.color || "#FFEB3B" // Yellow color
                });
    ...
    const saveToDatabase = async () => {
        ...
        try {
            setIsLoading(true);
            // Use /answers endpoint instead of /save-answer which doesn't exist
            const response = await fetch(`${API_BASE}/answers`, {
                method: 'POST', // POST to create a new answer
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    answer: modelResult.answer || modelResult.text || "No text detected",
            ...
            if (result.success || result.id || result.answer_id) {
                // Store the answer ID 
                if (result.id || result.answer_id) {
                    localStorage.setItem('yourRecentAnswerId', result.id || result.answer_id);
                }
                
                alert('Successfully added to the AI wall!');
                setScanningStep('start');
            } else {
                throw new Error('No success confirmation from server');
            }
        } catch (err) {
            console.error('Error saving to database:', err);
            alert('Error adding to wall: ' + (err.message || 'Unknown error'));
        } finally {
            setIsLoading(false);
        }
    };
