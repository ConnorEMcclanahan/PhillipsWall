    ...
    useEffect(() => {
        ...
        const updateClusterPositions = () => {
            ...
            const styles = clusteredAnswers.map((cluster) => {
                ...
                // Increase sizes overall - larger base size, larger growth per answer, higher max cap
                const baseSize = 25; // Increased from 20
                const maxSize = 90; // Increased from 60
                const growthPerAnswer = 12; // Increased from 8
                
                // Scale bubble size based on cluster size with more growth
                const size = Math.min(maxSize, baseSize + (cluster.size - 1) * growthPerAnswer);
        ...
        // Remove the interval to prevent the flying dots issue
