...
const AIQuestionsDisplay = () => {
    ...
    const [clusterStyles, setClusterStyles] = useState([]); // Add separate state for clusters
    ...
    useEffect(() => {
        ...
        // Remove the interval to stop flying around
        // const interval = setInterval(updateBubblePositions, 8000);
        // return () => clearInterval(interval);
    }, [answersData, getQuestionColor]);

    // Function to get the position and size of each cluster (separate state)
    useEffect(() => {
        ...
        const updateClusterPositions = () => {
            ...
            setClusterStyles(styles); // Use separate state
        };

        updateClusterPositions();
        // Remove interval to stop flying around
        ...
        <div className={styles.container}>
            <div className={styles.answersLayer}>
                <div className={styles.answersGrid}>
                    {/* Render individual bubbles for non-clustered answers */}
                    {answersData.map((answer, index) => {
                        // Check if this answer is part of any cluster
                        const isInCluster = clusteredAnswers.some(cluster => 
                            cluster.answers.some(clusterAnswer => clusterAnswer.answer_id === answer.answer_id)
                        );
                        
                        // Only render if not in a cluster
                        if (!isInCluster) {
                            return (
                                <div
                                    key={`individual-${answer.answer_id}`}
                    ...
                    {clusteredAnswers.map((cluster, index) => (    
                        <div
                            ...
                            style={clusterStyles[index]} // Use cluster styles
