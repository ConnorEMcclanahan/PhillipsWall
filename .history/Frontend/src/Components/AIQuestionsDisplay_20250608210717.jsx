...
const AIQuestionsDisplay = () => {
    ...
    const filterClustersBySeason = (seasonIndex) => {
        ...
        const filtered = clusteredAnswers.filter(cluster => {
            // Check all answers in the cluster for dates (in case first one doesn't have it)
            for (const answer of cluster.answers) {
                ...
                if (dateField) {
                    return getSeasonIndex(dateField) === seasonIndex;
                }
            }
            
            // If no answers in the cluster have dates, default to Summer 2025
            console.warn("No date field found in cluster:", cluster.id);
            return seasonIndex === 6;
        });
        
        console.log(`Filtered clusters for ${SEASONS[seasonIndex]}:`, filtered.length);
