...
const AIQuestionsDisplay = () => {
    ...
    useEffect(() => {
        ...
        const styles = clusteredAnswers.map((cluster) => {
            ...
            // UPDATED: Use consistent transformation that keeps bubbles inside the graph area
            // Map x and y values (which are between 0-1) to percentages between 10-90%
            // This gives a 10% margin on all sides
            const normalizedX = firstAnswer.x_axis_value * 80 + 10; // Maps 0-1 to 10-90%
            const normalizedY = firstAnswer.y_axis_value * 80 + 10; // Maps 0-1 to 10-90%
            ...
            return {
                ...
                left: `${normalizedX}%`,
                position: 'absolute',
                transform: 'translate(-50%, 50%)', // CHANGED: Correct transform for bottom positioning
                ...
                <div className={styles.answersGrid}>
                    {(filteredClusters.length > 0 ? filteredClusters : []).map((cluster, index) => {
                        ...
                        const style = styleIndex >= 0 ? bubbleStyles[styleIndex] : {
                            // UPDATED: Use the same positioning logic as in the bubbleStyles effect
                            bottom: `${cluster.answers[0].y_axis_value * 80 + 10}%`, 
                            left: `${cluster.answers[0].x_axis_value * 80 + 10}%`,
                            position: 'absolute',
                            transform: 'translate(-50%, 50%)', // CHANGED: Correct transform for bottom positioning
                            backgroundColor: getQuestionColor(cluster.questionId),
