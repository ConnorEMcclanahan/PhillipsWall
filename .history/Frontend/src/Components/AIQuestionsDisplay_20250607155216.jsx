...
const AIQuestionsDisplay = () => {
    ...
    // Update bubble positions for clusters
    useEffect(() => {
    ...
    // Handle cluster click - show individual answers in cluster or open question
    const handleClusterClick = async (cluster, event) => {
        if (cluster.size === 1) {
            // Single answer - open question directly
        ...
        // Also fetch question data for potential use
        try {
    ...
    // Handle individual bubble click (opens question overlay)
    ...
    // Set current month as active on mount
    useEffect(() => {
        const now = new Date();
    ...
    const handleTimelinePointClick = (monthIndex) => {
        ...
        // Filter logic can be added here based on selected month
    };
