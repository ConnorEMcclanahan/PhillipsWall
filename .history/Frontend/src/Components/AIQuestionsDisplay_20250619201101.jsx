    ...
    const renderExpandedCluster = () => {
        ...
        return (
            <>
                ...
                {/* REMOVED: Question bubble at the top */}
                ...
                {focusedCard !== null && (
                    <div style={{
                        ...
                        {/* Content starts below thumbtack */}
                        <div style={{
                            ...
                            {/* Question header - no bottom border */}
                            <div style={{
                                fontWeight: 'bold',
                                fontSize: '18px',
                                color: '#000',
                            ...
                            {/* Answer content area with responsive font size */}
                            <div style={{
                                fontSize: determineOptimalFontSize(
                                    clusterAnswers[focusedCard] ? getAnswerText(clusterAnswers[focusedCard], language) : "", 
                                    400, // Available width
                                    400  // Available height
                                ),
                                lineHeight: '1.4',
                                color: '#000',
                                fontFamily: "'Arial', sans-serif",
                ...
                {/* Card hand at bottom - larger cards with better focus behavior */}
                <div style={{
                    position: 'fixed',
                    bottom: '5%',
                    left: '50%',
                    transform: 'translateX(-50%)',
                    width: '95%',
                    height: '330px',  // INCREASED height for larger cards
                    zIndex: 1600,
                    display: 'flex',
                    justifyContent: 'center',
                    perspective: '1000px',
                    ...
                    <div style={{
                        ...
                        {clusterAnswers.map((answer, index) => {
                            ...
                            const maxAngle = Math.min(40, totalCards * 3); // Reduced angle for larger cards
                            const angle = (index - (totalCards - 1) / 2) * (maxAngle / totalCards);
                            const isHovered = hoveredCard === index;
                            const isFocused = focusedCard === index;
                            const otherCardIsFocused = focusedCard !== null && focusedCard !== index;
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        transform: isFocused
                                            ? 'translateY(-45%) translateX(0) scale(1.1) rotate(0deg)' // BRING TO CENTER
                                            : otherCardIsFocused 
                                                ? `translateX(${angle * 28}px) translateY(${isHovered ? -10 : 0}px) rotate(${angle}deg) scale(0.85) translateY(40px)` // HIDE a bit when another card is focused
                                                : `translateX(${angle * 28}px) translateY(${isHovered ? -30 : 0}px) rotate(${angle}deg)`,
                                        transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
                                        transformOrigin: 'bottom center',
                                        zIndex: isFocused ? 100 : (isHovered ? 50 + index : index),
                                        cursor: 'pointer',
                                        overflow: 'auto',
                                        opacity: otherCardIsFocused ? 0.6 : 1, // Dim other cards when one is focused
                                        display: 'flex',
                                        flexDirection: 'column',
                                    }}
                                >
                                    {/* Card content with question and answer text */}
                                    <div style={{
                                        position: 'relative',
                                        width: '100%',
                                        height: '100%',
                                        ...
                                        {/* Content container - starts below the thumbtack */}
                                        <div style={{
                                            marginTop: '35px', // Space for thumbtack
                                            padding: '0 15px',
                                            display: 'flex',
                                            flexDirection: 'column',
                                            flex: 1,
                                            ...
                                            <div style={{
                                                ...
                                                color: '#000',
                                                textAlign: 'center',
                                                marginBottom: '12px', // More space between question and answer
                                            }}>
                                                {questionText}
                                            </div>
                                            
                                            {/* Full answer text - no clipping */}
                                            <div style={{
                                                color: '#000',
                                                fontSize: '14px', // Larger font for better readability
                                                lineHeight: '1.5',
