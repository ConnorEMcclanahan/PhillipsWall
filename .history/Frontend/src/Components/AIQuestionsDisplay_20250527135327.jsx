    ...
    useEffect(() => {
        ...
        const updateBubblePositions = () => {
            const styles = answersData.map((answer) => {
                // Make sure values are numbers and within appropriate range
                const x = parseFloat(answer.x_axis_value) || 0;
                const y = parseFloat(answer.y_axis_value) || 0;
                
                // Scale to percentage (assuming x and y are between -1 and 1)
                const normalizedX = (x * 40) + 50; // Maps -1 to 10%, 1 to 90%
                const normalizedY = (y * 40) + 50; // Maps -1 to 10%, 1 to 90%
                ...
                return {
                    ...
                    width: '3rem',  // Explicitly set width
                    height: '3rem', // Explicitly set height
                    borderRadius: '50%', // Ensure circles
                    display: 'flex',
                    alignItems: 'center',
        ...
        // No need for interval if positions are static
