...
const ImagesCaptureLoad = () => {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            ...
            if (!modelResult.question_id) {
                // If question_id is missing, set a default value
                console.warn("Missing question_id in modelResult, using default question 1");
                modelResult.question_id = 1; // Default to first question
            }
            
            // Format the data exactly as your backend expects it
            const payload = {
                answer_text: modelResult.answer || "No text detected",
                question_id: parseInt(modelResult.question_id) || 1, // Ensure it's a number
