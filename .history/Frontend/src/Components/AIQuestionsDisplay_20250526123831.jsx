...
const AIQuestionsDisplay = () => {
    ...
    const handleQuestionClick = async (questionId, event) => {
        ...
        try {
            ...
            const data = await response.json();
            console.log("API Response:", data); // Debug: Log the response to see its structure
            ...
            const safeData = {
                ...
                color: data.color || getQuestionColor(questionsData.find(q => q.question_id === questionId)?.[language] || "")
            };

            // Ensure the current language exists in the data structure
            if (!safeData.question[language]) {
                safeData.question[language] = questionsData.find(q => q.question_id === questionId)?.[language] || "Question not available";
            }
            
            if (!safeData.answers[language]) {
                safeData.answers[language] = ["No answers available for this question"];
            }
            ...
            setTimeout(() => {
                ...
                setActiveQuestionData(safeData);
                setCurrentPage(1);
            }, TRANSITION_DURATION / 2);
        } catch (error) {
            console.error("Error fetching question details:", error);
            
            // Create fallback data from the selected question in the questionsData array
            const selectedQuestion = questionsData.find(q => q.question_id === questionId);
            
            if (selectedQuestion) {
                const fallbackData = {
                    question_id: questionId,
                    question: { [language]: selectedQuestion[language] },
                    answers: { [language]: ["Sorry, could not load answers for this question."] },
                    color: getQuestionColor(selectedQuestion[language])
                };
                
                setTimeout(() => {
                    gridElement?.classList.add(styles.zoomed);
                    setActiveQuestion(questionId);
                    setActiveQuestionData(fallbackData);
                    setCurrentPage(1);
                }, TRANSITION_DURATION / 2);
            } else {
                // If something goes wrong, clean up the UI state
                setIsZooming(false);
    ...
    useEffect(() => {
        if (!activeQuestionData || !activeQuestionData.answers || !activeQuestionData.answers[language]) {
            return;
        }
        ...
        <div className={styles.container}>
            <div className={styles.questionsLayer}>
                <div className={styles.questionsGrid}>
                    ...
                    {activeQuestionData && (
                        <div 
                            className={`${styles.overlay} ${isZooming ? styles.visible : ''}`}
                            onClick={(e) => {
                        ...
                        >
                            <div 
                                className={`${styles.contentWrapper} ${isZooming ? styles.entering : styles.leaving}`}
                            ...
                            >
                                ...
                                {activeQuestionData && activeQuestionData.question && activeQuestionData.question[language] && (
                                    <div
                                    ...
                                    {currentAnswers.map((answer, index, array) => (
                                        <div
                                            ...
                                            style={{
                                                ...
                                                backgroundColor: activeQuestionData?.color || "#1E88E5", // Add fallback color
                                                '--hover-color': activeQuestionData?.color || "#1E88E5", // Add fallback color
                                            }}
                                        >
                                            {answer}
                                        </div>
