    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.timeline} style={{
                ...
                position: 'relative', // Need this for absolute positioning of covers
            }}>
                {/* Cover for left end of timeline */}
                <div style={{
                    position: 'absolute',
                    left: '-5px', // Extend slightly past the edge
                    top: '50%',
                    width: '25px', // Wide enough to cover the tail
                    height: '3px', // Thick enough to cover the line
                    backgroundColor: '#fff', // Match page background
                    transform: 'translateY(-50%)',
                    zIndex: 5, // Above the line but below the dots
                }} />
                
                {/* Cover for right end of timeline */}
                <div style={{
                    position: 'absolute',
                    right: '-5px', // Extend slightly past the edge
                    top: '50%',
                    width: '25px', // Wide enough to cover the tail
                    height: '3px', // Thick enough to cover the line
                    backgroundColor: '#fff', // Match page background
                ...
                {/* Keep the existing timeline content unchanged */}
                {SEASONS.map((season, index) => (
                    <div
                        key={season}
                        className={styles.timelineItem}
                        onClick={() => handleTimelinePointClick(index)}
                        style={{ position: 'relative', zIndex: 10 }} // Ensure dots are above covers
                    >
                        <div 
                            className={`${styles.timelinePoint} ${activeSeason === index ? styles.active : ''}`}
                        />
                        <div className={`${styles.timelineMonth} ${activeSeason === index ? styles.current : ''}`}>
                            {season}
                        </div>
                    </div>
                ))}
