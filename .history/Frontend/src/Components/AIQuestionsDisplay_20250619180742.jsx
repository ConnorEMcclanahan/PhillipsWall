    ...
    useEffect(() => {
        ...
        const handleNewSubmission = (event) => {
            if (event.detail && event.detail.answer_id) {
                ...
                if (newId !== lastSeenAnswerId) {
                    ...
                    lastSeenAnswerId = newId; // Store in regular variable
        ...
        const fetchNewestAnswer = async () => {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const initialId = typeof data === 'number' ? data : (data?.answer_id);
                    
                    if (initialId && !youLabelDismissed && initialId !== lastSeenAnswerId) {
                        lastSeenAnswerId = initialId; // Store in regular variable
                        setNewestAnswerId(initialId);
                        console.log("✓ Setting initial newest answer ID:", initialId);
                    }
                }
            } catch (error) {
