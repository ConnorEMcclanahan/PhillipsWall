...
const ImagesCaptureLoad = () => {
    ...
    // Update the renderResults function to use the exact same color handling as AIQuestionsDisplay
    const renderResults = () => {
        if (!modelResult) {
            return (
                <Box className={styles.scanningContainer}>
                ...
                <Box className={styles.successContainer}>
                    ...
                    <Box className={styles.scannedPostItContainer}>
                        <Box 
                            ...
                            style={{ 
                                // Force use a fixed color to match AIQuestionsDisplay
                                backgroundColor: '#7EDDDE' // Use this aqua blue color that matches AIQuestionsDisplay
                                
                                // Alternative if you want to use the question ID to look up color:
                                // backgroundColor: questionsData.find(q => q.question_id === parseInt(modelResult.question_id))?.color || '#7EDDDE'
                            }}
                        >
                            {console.log("POST-IT COLOR FROM MODEL:", modelResult.color)}
                    ...
                    <Box className={styles.successMessageContainer}>
                        ...
                        <Box className={styles.successButtonsContainer}>
                            ...
                            <Button 
                                ...
                                // Changed to simply go back to start instead of saving
                                onClick={() => {
                                    // Still save to the database but don't wait for response
                                    if (hasValidText || modelResult.answer) {
                                        saveToDatabase();
                                    }
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    "Finish" // Changed from "Send to wall" to "Finish"
                                )}
                            </Button>
                        </Box>
                    </Box>
