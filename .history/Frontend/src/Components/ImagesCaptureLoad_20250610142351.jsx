...
const ImagesCaptureLoad = () => {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            ...
            // Create a timestamp for Summer 2025 (July)
            ...
            const payload = {
                ...
                scan_date: summerDate, // Add this field to ensure it appears in Summer
                x_axis_value: Math.random(),
                y_axis_value: Math.random(),
                answer_language: "en"  // Use proper language code for DB
            };
            
            console.log("Saving:", payload);
            
            // Add custom event to signal new post for AIQuestionsDisplay to detect
            const customEvent = new Event('newPostItSubmitted');
            ...
            try {
                ...
                answerId = result.id || result.answer_id;
            } catch (parseErr) {
                // Try to extract ID from response text
                console.log("Non-JSON response, trying to extract ID");
                const responseText = await response.text();
                const idMatch = responseText.match(/id[":=]\s*["']?(\d+)["']?/i);
                answerId = idMatch ? idMatch[1] : Date.now(); // Use timestamp as fallback ID
            }
            
            // Store the answer ID for the wall display to find
            if (answerId) {
                localStorage.setItem('yourRecentAnswerId', answerId);
                console.log("Saved answer ID to localStorage:", answerId);
                
                // Dispatch custom event to notify AIQuestionsDisplay
                window.dispatchEvent(customEvent);
            }
            
            alert('Successfully added to the wall!');
            resetScanState('start', false); // Return to start of capture flow
        } catch (err) {
