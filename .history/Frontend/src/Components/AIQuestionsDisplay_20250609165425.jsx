...
const AIQuestionsDisplay = () => {
    ...
    // Add this state for tracking the user's recent submission
    ...
    // Add a useEffect to check localStorage when the component mounts
    useEffect(() => {
    ...
    // Update the handleClusterClick function to remove the "YOU" label when clicked
    const handleClusterClick = (cluster) => {
        // If this is the user's cluster, remove the "YOU" label
                    ...
                    {(filteredClusters.length > 0 ? filteredClusters : []).map((cluster, index) => {
                        ...
                        return (
                            <div
                                ...
                                className={`${styles.bubbleCluster} ${isYourCluster ? styles.yourCluster : ''}`}
                                style={{
                                    left: `${cluster.position.x}%`,
                                    top: `${cluster.position.y}%`,
                                    zIndex: expandedCluster === cluster.id ? 100 : 2,
                                }}
                                onClick={() => handleClusterClick(cluster)}
                            >
                                {/* Show "YOU" label for the user's recent submission */}
                                {isYourCluster && (
                                ...
                                {/* Rest of your cluster rendering */}
                                {cluster.answers.slice(0, 1).map((answer, answerIndex) => (
                                    <div 
                                        key={`answer-${answer.answer_id || answerIndex}`}
                                        className={styles.bubbleAnswer}
                                    >
                                        {answer.answer}
                                    </div>
                                ))}
                                {cluster.answers.length > 1 && (
                                    <div className={styles.clusterCount}>+{cluster.answers.length - 1}</div>
                                )}
                            </div>
                        );
                    })}
