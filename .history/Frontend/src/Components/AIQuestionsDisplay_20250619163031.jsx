...
useEffect(() => {
        // Add a reference to track the last seen answer ID
const lastSeenAnswerId = useRef(null);
        ...
        const fetchNewestAnswer = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // If there's a valid ID and it's different from our last tracked one
                    if (newId && (newId !== lastSeenAnswerId.current)) {
                        console.log("New answer ID detected:", newId, "Previous:", lastSeenAnswerId.current);
                        
                        // Always update the reference to the last seen ID
                        lastSeenAnswerId.current = newId;
                        
                        // Always show the YOU label for genuinely new answers
                        setNewestAnswerId(newId);
                        setYouLabelDismissed(false); // Reset dismissed state for new answers
...
useEffect(() => {
    ...
    if (newestAnswerId) {
        ...
        const hideYouLabelTimer = setTimeout(() => {
            ...
            setNewestAnswerId(null); 
            setYouLabelDismissed(true); // Add this line to prevent reappearance
        }, 20000); // 20 seconds
