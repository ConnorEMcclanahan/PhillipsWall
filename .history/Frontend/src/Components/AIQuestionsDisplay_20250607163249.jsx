...
const AIQuestionsDisplay = () => {
    ...
    // Update handleClusterClick to properly handle single answers
    const handleClusterClick = async (cluster, event) => {
        if (cluster.size === 1) {
            console.log("Single answer clicked:", cluster.answers[0]);
            // Single answer - show question directly
            try {
                const response = await fetch(`http://localhost:5000/question/${cluster.questionId}`);
                const data = await response.json();
                setActiveQuestionData(data);
                setActiveQuestion(cluster.questionId);
                setIsZooming(true);
                ...
                // Show the bubble transition effect
                setTimeout(() => {
                    gridElement?.classList.add(styles.zoomed);
                    setCurrentPage(1); 
                }, TRANSITION_DURATION / 2);
        ...
        // Also fetch question data for the cluster
        try {
            const response = await fetch(`http://localhost:5000/question/${cluster.questionId}`);
            const data = await response.json();
            setActiveQuestionData(data);
            ...
            <>
                ...
                <div className={styles.expandedCluster}>
                    {clusterAnswers.map((answer, index) => {
                        ...
                        const answerText = (() => {
                            // Try all possible locations of the answer text
                            ...
                            // For answers in different languages
                            if (answer[language]) return answer[language];
                            
                            // Last resort fallbacks
                            return "Precision diagnostic";  // Hardcoded text matching your screenshot
                        })();
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    position: 'fixed', // Changed to fixed
                                    top: `calc(50% + ${offsetY}px)`, // Position from center
                                    left: `calc(50% + ${offsetX}px)`, // Position from center
                ...
                <div className={styles.answersGrid}>
                    ...
                    {clusteredAnswers.map((cluster, index) => (    
                        <div
                            ...
                            style={{
                                ...bubbleStyles[index],
                                // Fix opacity logic: only hide bubbles when a cluster is expanded
                                opacity: expandedCluster ? 0 : 1, // All bubbles visible when no cluster expanded
                                pointerEvents: expandedCluster ? 'none' : 'auto', // All bubbles clickable when no cluster expanded
