...
const AIQuestionsDisplay = () => {
    ...
    const lastSeenAnswerIdRef = useRef(null);
            ...
            <>
                ...
                <div 
                    ...
                    style={{
                        ...
                        zIndex: 1500, // Much higher than any bubble's z-index
                        opacity: 1,
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    padding: '0',
                                    borderRadius: '2px',
                                    zIndex: 1800, // Higher than overlay but lower than central question
...
useEffect(() => {
        ...
        const fetchNewestAnswer = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Only set the newest answer ID if it's different from the current one
                    // AND the user hasn't dismissed the label yet
                    if (newId && newId !== newestAnswerId && !youLabelDismissed) {
                        setNewestAnswerId(newId);
                        setYouLabelDismissed(false); // Reset dismissed state for genuinely new answers
                    }
                }
            } catch (error) {
    ...
    // Replace the newestAnswerId polling useEffect with this simplified version
useEffect(() => {
    // Track the last seen answer ID with a ref (persists between renders)
    const lastSeenAnswerIdRef = React.useRef(null);
    
    // Handle custom event fired when a new answer is submitted
    const handleNewSubmission = (event) => {
        if (event.detail && event.detail.answer_id) {
            console.log("New submission detected:", event.detail.answer_id);
            // Only update if this is a new ID we haven't seen before
            if (event.detail.answer_id !== lastSeenAnswerIdRef.current) {
                lastSeenAnswerIdRef.current = event.detail.answer_id;
                setNewestAnswerId(event.detail.answer_id);
    ...
    // Listen for new answer submitted events
    window.addEventListener('newAnswerSubmitted', handleNewSubmission);
    
    // Also listen for a custom "show my answer" event
    // This can be triggered from the submit form component
    window.addEventListener('showMyAnswer', handleNewSubmission);
    
    return () => {
        window.removeEventListener('newAnswerSubmitted', handleNewSubmission);
        window.removeEventListener('showMyAnswer', handleNewSubmission);
    };
}, []); // No dependencies needed - this only sets up the listener once

// Keep the 20-second timer effect
useEffect(() => {
    if (newestAnswerId) {
        const hideYouLabelTimer = setTimeout(() => {
            console.log("Hiding YOU label after 20 seconds");
            setNewestAnswerId(null);
