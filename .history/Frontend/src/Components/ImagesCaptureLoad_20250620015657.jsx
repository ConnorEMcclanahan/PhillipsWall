...
const ImagesCaptureLoad = () => {
    ...
    // Replace the renderCamera function
    const renderCamera = () => {
        return (
            <Box className={`${styles.scanningContainer} ${isLandscape ? styles.landscape : ''}`}>
                <Box className={styles.scanningContent}>
                    ...
                    <Box className={styles.cameraContainer} style={{position: 'relative'}}>
                        <video 
                            ref={videoRef}
    ...
    // Update the captureImage function to work with the video element
    const captureImage = () => {
    ...
    // Add this at the top level of your component, with all the other useEffects
    // This will handle the camera automatically when scanningStep changes
    useEffect(() => {
        if (scanningStep === 'scanning') {
            console.log("Starting camera from main useEffect");
            startCamera();
        } else {
            // Stop camera when not in scanning mode
            if (stream) {
                console.log("Stopping camera from main useEffect");
                stream.getTracks().forEach(track => track.stop());
                setStream(null);
            }
        }
    ...
    }, [scanningStep]); // This will run when scanningStep changes
