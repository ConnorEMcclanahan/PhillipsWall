...
const AIQuestionsDisplay = () => {
    ...
    const renderExpandedCluster = () => {
        ...
        const firstAnswer = expandedCluster.answers[0];
        const normalizedX = firstAnswer.x_axis_value * 50 + 50;
        const normalizedY = firstAnswer.y_axis_value * 50 + 50;
        ...
        return (
            <>
                ...
                {/* Content panel showing question and answers */}
                <div
                    className={styles.clusterContentPanel}
                    style={{
                        position: 'absolute',
                        top: '50%',
                        left: '50%',
                ...
                >
                    <div 
                        className={styles.clusterQuestionHeader}
                        style={{
                            ...
                            fontSize: '18px',
                            fontWeight: 'bold',
                            boxShadow: `0 0 20px ${getQuestionColor(expandedCluster.questionId)}, inset 0 0 40px ${getQuestionColor(expandedCluster.questionId)}`
                        }}
                    >
                        {activeQuestionData?.question[language] || 'Loading question...'}
                    </div>
                    
                    <div className={styles.clusterAnswersList}>
                        <h3 style={{marginBottom: '12px'}}>
                            {expandedCluster.answers.length} answers to this question:
                        </h3>
                        
                        {expandedCluster.answers.map((answer, index) => (
                            <div 
                                key={answer.answer_id}
                                className={styles.clusterAnswer}
                                style={{
                                    backgroundColor: 'rgba(240, 240, 240, 0.8)',
                                    padding: '12px 16px',
                                    borderRadius: '8px',
                                    marginBottom: '10px',
                                    borderLeft: `4px solid ${getQuestionColor(expandedCluster.questionId)}`
                                }}
                            >
                                <div>{answer.content || `Answer ${index + 1}`}</div>
                            </div>
                        ))}
                    </div>
