...
const AIQuestionsDisplay = () => {
    ...
    // Fetch questions with logging
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const response = await fetch('http://localhost:5000/questions');
                const rawData = await response.json();
                console.log("QUESTIONS API RESPONSE:", rawData);
                
                // Transform data if needed
                setQuestionsData(rawData);
            } catch (error) {
    ...
    // Fetch answers with logging
    useEffect(() => {
        const fetchAnswers = async () => {
            try {
                const response = await fetch('http://localhost:5000/answers');
                const rawData = await response.json();
                console.log("ANSWERS API RESPONSE:", rawData);
                
                if (!rawData || !Array.isArray(rawData) || rawData.length === 0) {
                    console.warn("No answers returned from API");
                    return;
                }
                
                // Transform if needed based on backend format
                const transformedData = rawData.map(item => ({
                    answer_id: item.answer_id || item.id || String(Math.random()),
                    question_id: item.question_id || item.questionId || "1",
                    x_axis_value: typeof item.x_axis_value !== 'undefined' ? item.x_axis_value : 
                                 (typeof item.x !== 'undefined' ? item.x : Math.random()),
                    y_axis_value: typeof item.y_axis_value !== 'undefined' ? item.y_axis_value : 
                                 (typeof item.y !== 'undefined' ? item.y : Math.random()),
                    answer: item.answer || item.text || item.content || "Answer text",
                    scan_date: item.scan_date || item.created_at || item.timestamp || new Date().toISOString()
                }));
                
                console.log("Transformed answers data:", transformedData);
                setAnswersData(transformedData);
            } catch (error) {
