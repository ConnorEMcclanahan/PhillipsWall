    ...
    useEffect(() => {
        ...
        const updateClusterPositions = () => {
            ...
            const styles = clusteredAnswers.map((cluster) => {
                ...
                // Special size for clusters with 8 answers
                let size;
                if (cluster.size === 8) {
                    // Double the normal size for clusters with exactly 8 answers
                    const normalSize = Math.min(60, 20 + (cluster.size - 1) * 8);
                    size = normalSize * 2; // Double size
                } else {
                    // Scale bubble size based on cluster size (min 20px, max 60px)
                    const baseSize = 20;
                    const maxSize = 60;
                    size = Math.min(maxSize, baseSize + (cluster.size - 1) * 8);
                }
                ...
                return {
                    ...
                    // Add special styling for clusters with 8 answers
                    border: cluster.size === 8 ? '3px solid white' : 'none',
                    boxShadow: cluster.size === 8 ? '0 0 15px rgba(255, 255, 255, 0.5)' : '0 2px 8px rgba(0,0,0,0.2)'
                };
            });
        ...
        // Remove the interval to prevent the flying dots issue we fixed earlier
