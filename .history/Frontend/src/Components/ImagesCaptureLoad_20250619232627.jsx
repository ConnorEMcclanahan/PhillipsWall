...
const ImagesCaptureLoad = () => {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                // Make sure color is explicitly sent with high priority
                color: modelResult.color,
            ...
            console.log("Saving post-it with color:", modelResult.color);
    ...
    // Function to get question color, prioritizing detected model color
    const getQuestionColor = useCallback((questionId) => {
        // First priority: directly use the color detected from the image
        if (modelResult?.color) {
            console.log("Using detected color from image:", modelResult.color);
            return modelResult.color;
        }
        
        // Second priority: check questions data
        const question = questionsData.find(q => q.question_id === questionId);
        if (question?.color) {
            console.log("Using color from questions data:", question.color);
            return question.color;
        }
        
        // Fallback
        return '#feff9c'; // Default yellow post-it color
    }, [questionsData, modelResult]);
    ...
    const renderResults = () => {
        ...
        const questionColor = getQuestionColor(modelResult.question_id);
                ...
                <Box className={styles.successContainer}>
                    ...
                    <Box className={styles.scannedPostItContainer}>
                        <Box 
                            ...
                            style={{ 
                                // Use modelResult.color directly with no fallbacks or overrides
                                backgroundColor: modelResult.color
                            }}
                        >
                            ...
                            {/* Add logging to verify the color */}
                            <Box sx={{ display: 'none' }}>
                                {console.log("FINAL COLOR SHOWN:", modelResult.color)}
                            </Box>
