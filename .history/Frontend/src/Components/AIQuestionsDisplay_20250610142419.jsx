...
const AIQuestionsDisplay = () => {
    ...
    // Add this useEffect near your other event listeners
    useEffect(() => {
        // Listen for custom event from ImagesCaptureLoad
        const handleNewPost = () => {
            console.log("New post detected!");
            // Force fetch answers immediately
            fetchAnswers();
            
            // Always set to Summer 2025
            setActiveSeason(6); // Index for Summer 2025
            
            // Check for your recent submission
            const storedId = localStorage.getItem('yourRecentAnswerId');
            if (storedId) {
                setYourRecentAnswerId(storedId);
                console.log("Found your recent post:", storedId);
            }
        };
        
        // Define fetchAnswers function here
        const fetchAnswers = async () => {
            try {
                const response = await fetch(`${API_BASE}/answers`);
                const data = await response.json();
                setAnswersData(data);
                console.log("Refreshed answers data");
            } catch (error) {
                console.error("Error fetching answers:", error);
            }
        };

        window.addEventListener('newPostItSubmitted', handleNewPost);
        return () => window.removeEventListener('newPostItSubmitted', handleNewPost);
    }, []);
