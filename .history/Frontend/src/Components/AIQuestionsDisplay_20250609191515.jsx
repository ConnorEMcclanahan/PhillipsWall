        ...
        const fetchAnswers = async () => {
            try {
                ...
                const transformedData = Array.isArray(rawData) ? rawData.map(item => ({
                    answer_id: item.id || item.answer_id || `temp-${Math.random()}`,
                    question_id: item.question_id || item.questionId || "1",
                    x_axis_value: item.x_axis_value || item.x || Math.random(),
                    y_axis_value: item.y_axis_value || item.y || Math.random(),
                    answer: item.answer || item.text || "Answer content",
