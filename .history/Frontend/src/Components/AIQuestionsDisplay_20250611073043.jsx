    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data && data.answer_id) {
                const newId = data.answer_id;
                setNewestAnswerId(newId);
                console.log("✅ Newest answer ID set to:", newId);
                
                // Log existing answers to check if this ID exists in our data
                console.log("Checking if this ID exists in our answer data...");
                const found = answersData.some(answer => answer.answer_id === newId);
                console.log(`ID ${newId} exists in current answers data: ${found}`);
                
                // Also check filtered clusters
                console.log("Available answer IDs in filtered clusters:", 
                    filteredClusters.map(cluster => 
                        cluster.answers.map(a => a.answer_id)
                    ).flat()
                );
            } else {
                console.error("❌ No answer_id in response:", data);
            }
        } else {
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    boxShadow: isYourCluster ? '0 0 15px rgba(255,255,255,0.7)' : 'none',
                                    animation: isYourCluster ? 'pulse 2s infinite' : 'none'
                                }}
                                ...
                                {isYourCluster && (
                                    <div 
                                        ...
                                        style={{
                                            ...
                                            top: '-5px',
                                            left: '100%',
                                            backgroundColor: '#FAD02C', // Yellow background
                                            color: 'black',
                                            fontWeight: 'bold',
