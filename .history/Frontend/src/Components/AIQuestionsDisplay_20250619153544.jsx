...
const AIQuestionsDisplay = () => {
    ...
    // Add this state to track if user has dismissed their "YOU" label
const [youLabelDismissed, setYouLabelDismissed] = useState(false);
    ...
    const handleClusterClick = async (cluster, event) => {
        ...
        if (isYourCluster) {
            // Clear the newestAnswerId and mark as dismissed
        ...
        const fetchNewestAnswer = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Only set the newest answer ID if:
                    // 1. We have a valid ID
                    // 2. It's different from the current one OR
                    // 3. User hasn't dismissed the label yet
                    if (newId && (newId !== newestAnswerId) && !youLabelDismissed) {
                        setNewestAnswerId(newId);
                        setYouLabelDismissed(false); // Reset the dismissed state for new answers
...
>
    {isYourCluster && (
        <div 
            ...
            onClick={(e) => {
                e.stopPropagation(); // Prevent triggering the cluster click
                setNewestAnswerId(null);
                setYouLabelDismissed(true);
            }}
            style={{
