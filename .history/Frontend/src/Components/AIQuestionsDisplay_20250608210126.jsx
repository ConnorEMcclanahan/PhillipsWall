...
const AIQuestionsDisplay = () => {
    ...
    // Add the function to filter clusters by season
    const filterClustersBySeason = (seasonIndex) => {
        // For our simulation, use the answer_id to deterministically assign to seasons
        // This ensures consistent filtering behavior when clicking the same season
        const filtered = clusteredAnswers.filter(cluster => {
            // Use cluster.id as a deterministic way to assign to seasons
            const hash = parseInt(cluster.id.replace(/\D/g, '')) || 0;
            return hash % 8 === seasonIndex;
        });
