...
const AIQuestionsDisplay = () => {
    ...
    useEffect(() => {
        // Use a ref to persist the last seen ID between renders
        ...
        const handleNewSubmission = (event) => {
            if (event.detail && event.detail.answer_id) {
                ...
                console.log("New submission detected:", newId);
                
                // If this is a different ID than the last one we saw
                if (newId !== lastSeenIdRef.current) {
                    console.log("New answer ID differs from last seen:", newId, "vs", lastSeenIdRef.current);
                    lastSeenIdRef.current = newId;
                    setNewestAnswerId(newId);
                    setYouLabelDismissed(false); // Reset this flag for new answers
                    console.log("YOU label should appear for new answer ID:", newId);
                }
            }
        };
        ...
        const fetchNewestAnswer = async () => {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const newId = typeof data === 'number' ? data : (data?.answer_id);
                    
                    if (newId && !youLabelDismissed && newId !== lastSeenIdRef.current) {
                        lastSeenIdRef.current = newId;
                        setNewestAnswerId(newId);
                        console.log("✓ Setting initial newest answer ID:", newId);
                    }
                }
            } catch (error) {
    ...
    // KEEP the existing 20-second timer effect as is:
    useEffect(() => {
        if (newestAnswerId) {
            ...
            const hideYouLabelTimer = setTimeout(() => {
                ...
                setNewestAnswerId(null); 
                setYouLabelDismissed(true); // Prevent reappearance
            }, 20000); // 20 seconds
