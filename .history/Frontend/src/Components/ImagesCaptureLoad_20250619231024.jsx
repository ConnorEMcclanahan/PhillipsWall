...
const ImagesCaptureLoad = () => {
    ...
    // Function to get question color, matching AIQuestionsDisplay exactly
    const getQuestionColor = useCallback((questionId) => {
        // Ensure we're using the exact same logic as AIQuestionsDisplay
        const question = questionsData.find(q => q.question_id === questionId);
        if (question?.color) {
            return question.color;
        }
        
        // For direct color from the model result
        if (modelResult?.question_id === questionId && modelResult?.color) {
