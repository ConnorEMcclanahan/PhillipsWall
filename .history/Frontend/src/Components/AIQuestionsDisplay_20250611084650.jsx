...
// Move this outside the component to avoid hook-related issues
// Move this outside the component to avoid hook-related issues
...
// Add this function to generate subtle animation values for cards
const generateCardAnimation = (index) => {
    // Create unique but subtle animations for each card
    const moveX = Math.sin(index * 0.7) * 4; // -4px to 4px horizontal movement
    const moveY = Math.cos(index * 0.5) * 3; // -3px to 3px vertical movement
    const animDuration = 5 + (index % 3) * 2; // Animation duration between 5-9 seconds
    const animDelay = index * 0.3; // Stagger animation start
    
    return {
        animation: `float ${animDuration}s ease-in-out ${animDelay}s infinite alternate`,
        // Apply the movement to the transform property (preserving the existing translate)
...
const AIQuestionsDisplay = () => {
    ...
    <div
        ...
        style={{
            ...
            opacity: 1,
            // Replace the hardcoded animation with the function
            ...generateCardAnimation(index)
        }}
    >
...
useEffect(() => {
    ...
    style={{
        ...
        // Remove the boxShadow and animation effects
        border: isYourCluster ? '3px solid black' : 'none', // Add black border instead
    }}
