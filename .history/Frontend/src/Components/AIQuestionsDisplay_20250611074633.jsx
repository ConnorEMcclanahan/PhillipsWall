    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data && data.answer_id) {
                ...
                console.log("Latest answer ID:", data.answer_id);
            }
        } else {
            ...
            <div className={styles.answersLayer}>
                <div className={styles.answersGrid}>
                    {(filteredClusters.length > 0 ? filteredClusters : []).map((cluster, index) => {
                        ...
                        return (
                           // Update the cluster style to remove the animation and pulsing
<div
    key={cluster.id}
    ...
    style={{
        ...
        // Removed animation and boxShadow that caused yellow flashing
    }}
...
>
    {isYourCluster && (
        <>
            {/* Sized black circle border at the edge of the bubble */}
            <div style={{
                position: 'absolute',
                top: '50%',
                right: '-2px',
                // Dynamically size the circle based on the cluster size
                width: `${Math.max(10, Math.min(30, cluster.size * 3))}px`,  
                height: `${Math.max(10, Math.min(30, cluster.size * 3))}px`,
                borderRadius: '50%',
            ...
            <div style={{
                position: 'absolute',
                top: '50%',
                right: `${Math.max(10, Math.min(30, cluster.size * 3)) / 2}px`,
                width: '20px',
                height: '2px',
            ...
            <div 
                ...
                style={{
                    position: 'absolute',
                    top: '50%',
                    right: `${Math.max(10, Math.min(30, cluster.size * 3)) / 2 + 20}px`,
                    transform: 'translateY(-50%)',
                    backgroundColor: 'black',
                    color: 'white',
                    fontWeight: 'bold',
