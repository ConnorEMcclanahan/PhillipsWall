...
const ImagesCaptureLoad = () => {
    ...
    const saveToDatabase = async () => {
        ...
        try {
            ...
            // IMPORTANT: Create a timestamp for Summer 2025 (July)
            const summerDate = new Date(2025, 6, 15).toISOString(); // July 15, 2025
            
            // Format the data exactly as the backend expects it with REQUIRED FIELDS
            const payload = {
            ...
            // FIX: Use correct URL and remove credentials option that was causing 405
            const response = await fetch(`${API_BASE}/answers`, {
                ...
                // REMOVE this line which was causing CORS preflight issues
                // credentials: 'omit'
            });
            ...
            // Store answer ID and dispatch event
            if (result && (result.id || result.answer_id)) {
                ...
                console.log("Saved answer ID:", answerId);
                
                // Create and dispatch custom event for AIQuestionsDisplay
                const customEvent = new CustomEvent('newPostItSubmitted');
                window.dispatchEvent(customEvent);
                
                alert('Successfully added to the wall!');
                resetScanState('start', false); // Return to start
            } else {
                throw new Error("Missing answer ID in response");
            }
        } catch (err) {
