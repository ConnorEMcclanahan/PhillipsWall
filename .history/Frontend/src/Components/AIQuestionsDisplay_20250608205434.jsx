...
const AIQuestionsDisplay = () => {
    ...
    // Replace MONTHS with SEASONS array
    const SEASONS = [
    ...
    const [activeSeason, setActiveSeason] = useState(0); // Default to first season
    ...
    // Add a function to filter answers by season
    // This is a simplification since you mentioned your data might not have dates
    const filterAnswersBySeason = (seasonIndex) => {
        // For demonstration, let's filter answers randomly based on season index
        // You'll replace this with actual date filtering when you have the data
        if (!answersData.length) return;
        
        // Either use date filtering if you have created_at dates:
        // const filteredAnswers = answersData.filter(answer => {
        //     const date = new Date(answer.created_at);
        //     const month = date.getMonth();
        //     const year = date.getFullYear();
        //     
        //     // Map month/year to season index
        //     const answerSeasonIndex = getSeasonIndex(month, year);
        //     return answerSeasonIndex === seasonIndex;
        // });
        
        // Or simulate filtering with a deterministic random sample:
        const seed = seasonIndex + 1; // Use season as seed
        const filteredAnswers = answersData.filter(answer => {
            // Use answer_id hash to determine if this answer belongs in this season
            const hash = answer.answer_id * 713; // Simple hash function
            return (hash % 8) === seasonIndex; // 8 seasons total
        });
        
        console.log(`Filtering for ${SEASONS[seasonIndex]}: ${filteredAnswers.length} answers`);
        
        // Re-cluster the filtered answers 
        const clusters = clusterFilteredAnswers(filteredAnswers);
        setFilteredClusters(clusters);
    };

    // Duplicate clusterAnswers function but for filtered data
    const clusterFilteredAnswers = (filteredData) => {
        if (!filteredData.length) return [];

        const clusters = [];
        const processed = new Set();

        filteredData.forEach((answer, index) => {
            if (processed.has(index)) return;

            const cluster = {
                id: `filtered-cluster-${clusters.length}`,
                questionId: answer.question_id,
                answers: [answer],
                centerX: answer.x_axis_value,
                centerY: answer.y_axis_value,
            ...
            // Find nearby answers with the same question_id
            filteredData.forEach((otherAnswer, otherIndex) => {
                if (
                    otherIndex !== index &&
                    !processed.has(otherIndex) &&
                    otherAnswer.question_id === answer.question_id
    ...
    // Update the handleTimelinePointClick to filter data
    const handleTimelinePointClick = (seasonIndex) => {
        setActiveSeason(seasonIndex);
        filterAnswersBySeason(seasonIndex);
    };
                        ...
                        <div
                            ...
                            style={{
                                ...(filteredClusters.length > 0 ? 
                                    // For filtered clusters, calculate styles on the fly
                                    {
                                    ...
                                    // For regular display, use precalculated bubbleStyles
                                    bubbleStyles[index]),
                                opacity: 1,
                                pointerEvents: 'auto', 
                                cursor: 'pointer',
                                zIndex: 10 // Ensure bubbles are clickable by being above other elements
                            }}
