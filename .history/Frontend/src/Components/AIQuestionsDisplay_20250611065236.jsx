...
useEffect(() => {
    // Simple function to get newest answer ID - no fallback sorting
    // Fix the fetchNewestAnswer function
const fetchNewestAnswer = async () => {
    try {
        const response = await fetch(`${API_BASE}/get_latest_answer_id`);
        if (response.ok) {
            const data = await response.json();
            if (data && data.answer_id) {
                // Change this line from data.get_latest_answer_id to data.answer_id
