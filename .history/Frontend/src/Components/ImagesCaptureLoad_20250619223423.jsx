...
const ImagesCaptureLoad = () => {
    ...
    const toggleFullScreen = async (isUserAction = true) => {
        try {
            if (!document.fullscreenElement) {
                // Only enter fullscreen if the user hasn't manually exited it before
                if (!isUserAction && userExitedFullscreen) {
                    return; // Don't re-enter fullscreen automatically
                }
                ...
                setUserExitedFullscreen(false); // Reset when user enters fullscreen
            } else {
                ...
                // Only track user exiting fullscreen if it was their action
                if (isUserAction) {
    ...
    useEffect(() => {
        if (scanningStep === 'scanning' && !userExitedFullscreen) {
            // Small delay to ensure UI is ready
            const timer = setTimeout(() => {
                toggleFullScreen(false).catch(err => {
    ...
    // Add this useEffect at the top of your component to enter fullscreen immediately on mount
    useEffect(() => {
        // Small delay for component to fully render before going fullscreen
        const initialFullscreenTimer = setTimeout(() => {
            toggleFullScreen(false).catch(err => {
                console.log("Initial fullscreen failed:", err);
            });
        }, 500);
        
        return () => clearTimeout(initialFullscreenTimer);
    }, []); // Empty dependency array to only run once on mount

    // Replace the existing useEffect for the scanning step with this version
    // that maintains fullscreen throughout all steps
    useEffect(() => {
        // If we've lost fullscreen status for any reason, try to restore it
        // (unless the user explicitly exited)
        if (!document.fullscreenElement && !userExitedFullscreen) {
            const restoreFullscreenTimer = setTimeout(() => {
                toggleFullScreen(false).catch(err => {
                    console.log("Auto restore fullscreen failed:", err);
                });
            }, 500);
            
            return () => clearTimeout(restoreFullscreenTimer);
        }
    }, [scanningStep, isFullScreen, userExitedFullscreen]);

    // Modify your fullscreen management to be more aggressive at maintaining fullscreen state

    // Update this function to always check fullscreen status after step changes
    useEffect(() => {
      // Set a short delay to ensure DOM updates are complete
      const checkFullscreenTimer = setTimeout(() => {
        // Check if we're not in fullscreen but should be
        if (!document.fullscreenElement && !userExitedFullscreen) {
          console.log("Detected fullscreen was lost during navigation, restoring...");
          toggleFullScreen(false).catch(err => {
            console.log("Fullscreen restoration failed:", err);
          });
        }
      }, 300);
      
      return () => clearTimeout(checkFullscreenTimer);
    }, [scanningStep]); // Run this whenever the scanning step changes

    // Add this function to force-check fullscreen status periodically
    useEffect(() => {
      // Check every 2 seconds if fullscreen should be restored
      const fullscreenKeepAlive = setInterval(() => {
        if (!document.fullscreenElement && !userExitedFullscreen) {
          console.log("Periodic fullscreen check - restoring fullscreen");
          toggleFullScreen(false).catch(err => {
            // Fail silently
          });
        }
      ...
      return () => clearInterval(fullscreenKeepAlive);
