            ...
            <div className={styles.answersLayer}>
                <div className={styles.answersGrid}>
                    {(filteredClusters.length > 0 ? filteredClusters : []).map((cluster, index) => {
                        // Find the newest answer in this cluster using answer_date timestamp
                        const newestAnswer = [...cluster.answers].sort((a, b) => {
                            // Get timestamps from both answers
                            const dateA = new Date(a.answer_date || 0).getTime();
                            const dateB = new Date(b.answer_date || 0).getTime();
                            
                            // Return negative if B is newer (for descending order)
                            return dateB - dateA; // This was your bug - dateB wasn't defined!
                        })[0];
                        
                        // Check if this is the user's recent submission
                        const isYourCluster = yourRecentAnswerId && 
                            newestAnswer.answer_id === yourRecentAnswerId;
