...
const AIQuestionsDisplay = () => {
    ...
    const getSeasonIndex = (dateString) => {
        if (!dateString) return 6; // Default to Summer 2025 if no date
        
        const date = new Date(dateString);
        const month = date.getMonth(); // 0-indexed
        const year = date.getFullYear();
        
        // Return seasons only from Winter 2024 to Summer 2025
        if (year < 2024) return 0; // Default to Winter 2024 for older dates
        if (year > 2025) return 6; // Default to Summer 2025 for future dates
        
        if (year === 2024) {
            if (month <= 2) return 0; // Winter 2024 (Dec 2023, Jan-Feb 2024)
            if (month <= 5) return 1; // Spring 2024 (Mar-May 2024)
            if (month <= 8) return 2; // Summer 2024 (Jun-Aug 2024)
            return 3; // Fall 2024 (Sep-Nov 2024)
        }
        
        if (year === 2025) {
            if (month <= 2) return 4; // Winter 2025 (Dec 2024, Jan-Feb 2025)
            if (month <= 5) return 5; // Spring 2025 (Mar-May 2025)
            return 6; // Summer 2025 (Jun-Aug 2025)
        }
    };

    // Update the filterClustersBySeason function to work better with your data
    ...
    const filterClustersBySeason = (seasonIndex) => {
        ...
        // Check if any of the answers actually have dates
        const hasAnyDates = clusteredAnswers.some(cluster => {
            return cluster.answers.some(answer => 
                answer.created_at || answer.createdAt || answer.date || 
                answer.timestamp || answer.scan_date
            );
        });
        ...
        if (!hasAnyDates) {
            // If no dates found in data, use a deterministic approach based on answer_id
            console.log("No date fields found in any answers, using deterministic distribution");
            filtered = clusteredAnswers.filter(cluster => {
                // Use the numeric part of the ID to assign to seasons (deterministic)
                const idNumber = parseInt(cluster.id.replace(/\D/g, '')) || 0;
                return idNumber % SEASONS.length === seasonIndex;
            });
        } else {
            // If dates exist, use them
            filtered = clusteredAnswers.filter(cluster => {
                for (const answer of cluster.answers) {
                    const dateField = answer.created_at || 
                                    answer.createdAt || 
                                    answer.date || 
                                    answer.timestamp ||
                                    answer.scan_date;
                    ...
                    if (dateField) {
                        try {
                            ...
                            console.log(`Parsed date ${dateField} to season: ${parsed}`);
                            return parsed === seasonIndex;
                        } catch (e) {
                ...
                // Default assignment if no valid date
                return seasonIndex === 6; // Default to Summer 2025
            });
        }
        ...
        // Force re-render of the clusters
        setBubbleStyles(prevStyles => [...prevStyles]);
    };
        ...
        <div className={styles.container}>
            <div className={styles.answersLayer}>
                <div className={styles.answersGrid}>
                    {/* Use filteredClusters when available, otherwise fallback to clusteredAnswers */}
                    {(filteredClusters.length > 0 ? filteredClusters : clusteredAnswers).map((cluster, index) => {
                        // Get the correct styles based on whether we're viewing filtered or all clusters
                        const isFiltered = filteredClusters.length > 0;
                        const styleIndex = isFiltered ? 
                            // For filtered view, find original index in clusteredAnswers
                            clusteredAnswers.findIndex(c => c.id === cluster.id) : 
                            // For normal view, use direct index
                            index;
                            
                        // Use existing style if found, otherwise generate one
                        const style = bubbleStyles[styleIndex] || {
