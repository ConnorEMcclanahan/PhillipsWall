...
const AIQuestionsDisplay = () => {
    ...
    // Improved date parsing function with better debugging
    const getSeasonIndex = (dateString) => {
        if (!dateString) {
            console.log("Empty date field");
            return null; // Return null for empty dates instead of defaulting
        }
        ...
        try {
            ...
            if (isNaN(date.getTime())) {
                console.log(`Invalid date parse for: ${dateString}`);
                return null;
            }
        } catch (e) {
            console.log(`Date parsing error for: ${dateString}`, e);
            return null;
        }
        ...
        console.log(`Date parsed: ${dateString} → ${year}-${month + 1} (${date.toISOString()})`);
        
        // Return seasons only from Winter 2024 to Summer 2025
        if (year < 2024) {
            console.log(`Date too old: ${dateString}`);
            return null; // Don't show data from before 2024
        }
        if (year > 2025) {
            console.log(`Date too future: ${dateString}`);
            return null; // Don't show data from beyond 2025
        }
        
        let seasonIndex;
        if (year === 2024) {
            if (month <= 2) seasonIndex = 0; // Winter 2024 (Jan-Feb-Mar 2024)
            else if (month <= 5) seasonIndex = 1; // Spring 2024 (Apr-May-Jun 2024)
            else if (month <= 8) seasonIndex = 2; // Summer 2024 (Jul-Aug-Sep 2024)
            else seasonIndex = 3; // Fall 2024 (Oct-Nov-Dec 2024)
        } else if (year === 2025) {
            if (month <= 2) seasonIndex = 4; // Winter 2025 (Jan-Feb-Mar 2025)
            else if (month <= 5) seasonIndex = 5; // Spring 2025 (Apr-May-Jun 2025)
            else seasonIndex = 6; // Summer 2025 (Jul-Aug-Sep 2025)
        }
        
        console.log(`Season index for ${dateString}: ${seasonIndex} (${SEASONS[seasonIndex]})`);
        return seasonIndex;
    };
