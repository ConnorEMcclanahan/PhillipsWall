...
const LanguageToggle = ({ onToggle, currentLang }) => {
    return (
        <div className={styles.wallLanguageToggle} style={{
            position: 'absolute',
            right: '-190px', // Move further to the right
            bottom: '-2px',  // Align with timeline's vertical position
            zIndex: 20,
        }}>
            <Box 
                sx={{
...
const AIQuestionsDisplay = () => {
    ...
    // Helper function to get answer text consistently with proper language handling
    const getAnswerText = (answer, language) => {
        if (!answer) return "";
        
        // First try to get text in the requested language
        if (language === 'nl') {
            if (answer.nl) return answer.nl;
            if (answer.dutch) return answer.dutch;
        } else {
            if (answer.en) return answer.en; 
            if (answer.english) return answer.english;
        }
        
        // If we don't have the requested language, try any available field
...
return (
                    ...
                    {filteredClusters.length === 0 && (
                        <div style={{
                            ...
                            {translate('noAnswersAvailable', 'No answers available for')} {SEASONS[activeSeason]}
                        </div>
                    )}
                </div>
            ...
            <div className={styles.timeline} style={{
                ...
                {/* Language toggle positioned relative to timeline */}
                <LanguageToggle 
                    onToggle={(newLang) => {
                        setLanguage(newLang);
