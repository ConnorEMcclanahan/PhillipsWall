...
const AIQuestionsDisplay = () => {
    ...
    // Update renderExpandedCluster to use the first answer's position the first answer's position
    const renderExpandedCluster = () => {
        ...
        // Use the first answer's position instead of the calculated centerhe first answer's position instead of the calculated center
        const firstAnswer = clusterAnswers[0];
        const normalizedX = firstAnswer.x_axis_value * 50 + 50;0 + 50;
        const normalizedY = firstAnswer.y_axis_value * 50 + 50;

        return (
            <div className={styles.expandedCluster}>
                {clusterAnswers.map((answer, index) => {                {clusterAnswers.map((answer, index) => {
                    // Calculate position around cluster center
                    const angle = (index / clusterAnswers.length) * 2 * Math.PI;th.PI;
                    const offsetX = Math.cos(angle) * radius;                    const offsetX = Math.cos(angle) * radius;
                    const offsetY = Math.sin(angle) * radius;fsetY = Math.sin(angle) * radius;

                    return (
                        <div
                            key={answer.answer_id}wer.answer_id}
                            className={styles.expandedAnswer}
                            style={{
                                bottom: `calc(${normalizedY}% + ${offsetY}px)`,lizedY}% + ${offsetY}px)`,
                                left: `calc(${normalizedX}% + ${offsetX}px)`,setX}px)`,
                                // Rest of styles remain the same
                                position: 'absolute',olute',
                                transform: 'translate(-50%, -50%)',nslate(-50%, -50%)',
                                backgroundColor: getQuestionColor(answer.question_id),QuestionColor(answer.question_id),
                                width: '25px',
                                height: '25px',
                                borderRadius: '50%',s: '50%',
                                cursor: 'pointer',
                                zIndex: 20,
                                border: '2px solid white',  border: '2px solid white',
                                boxShadow: '0 2px 8px rgba(0,0,0,0.2)' 2px 8px rgba(0,0,0,0.2)'
                            }}
                            onClick={(e) => {
                                e.stopPropagation();  e.stopPropagation();
                                handleBubbleClick(answer.answer_id, answer.question_id, e);answer_id, answer.question_id, e);
                            }}  }}
                            title={`Answer ${index + 1}`}      title={`Answer ${index + 1}`}
                        />     />
                    );    );
                })}
                ...
                <div
                    ...
                    style={{
                        bottom: `calc(${normalizedY}% + 100px)`,lizedY}% + 100px)`,
                        left: `${normalizedX}%`,
                        position: 'absolute',
                        transform: 'translateX(-50%)',nslateX(-50%)',
                        backgroundColor: 'rgba(0,0,0,0.8)',ba(0,0,0,0.8)',
                        color: 'white',
                        padding: '8px 12px',x',
                        borderRadius: '8px',
                        fontSize: '12px','12px',
                ...
                >
                    {clusterAnswers.length} answers clustered here  {clusterAnswers.length} answers clustered here
                </div>      </div>
            </div>      </div>
        );        );
    };
    ...
    return (
        <div className={styles.container}>
            <div className={styles.answersLayer}>
                <div className={styles.answersGrid}>me={styles.answersGrid}>
                    {clusteredAnswers.map((cluster, index) => (    luster, index) => (    
                        <div
                            key={cluster.id}
                            className={`${styles.answerItem} ${cluster.size > 1 ? styles.cluster : ''}`} ${cluster.size > 1 ? styles.cluster : ''}`}
                            data-color={getQuestionColor(cluster.questionId)}r(cluster.questionId)}
                            data-cluster-size={cluster.size}
                            style={bubbleStyles[index]}
                            onClick={(e) => handleClusterClick(cluster, e)}   onClick={(e) => handleClusterClick(cluster, e)}
                            title={cluster.size > 1 ? `${cluster.size} answers` : 'Single answer'}1 ? `${cluster.size} answers` : 'Single answer'}
                        >
                            {cluster.size > 1 && (
                                <span className={styles.clusterCount}>lassName={styles.clusterCount}>
                ...
                {/* Render expanded cluster */}* Render expanded cluster */}
                {renderExpandedCluster()}                {renderExpandedCluster()}
            </div>

            <div className={styles.gridLines}>
                <div className={`${styles.axisLabel} ${styles.labelTop}`}>AI enthusiast</div>/div>
                <div className={`${styles.axisLabel} ${styles.labelBottom}`}>AI skeptic</div>          <div className={`${styles.axisLabel} ${styles.labelBottom}`}>AI skeptic</div>
                <div className={`${styles.axisLabel} ${styles.labelLeft}`}>Little scared    <div className={`${styles.axisLabel} ${styles.labelLeft}`}>Little scared
of the
future</div></div>
                <div className={`${styles.axisLabel} ${styles.labelRight}`}>Looking bright    <div className={`${styles.axisLabel} ${styles.labelRight}`}>Looking bright
to the
future</div>future</div>
            </div>

            <div className={styles.timeline}>me={styles.timeline}>
                {MONTHS.map((month, index) => ( index) => (
                    <div
                        key={month}
                        className={styles.timelineItem}   className={styles.timelineItem}
                        onClick={() => handleTimelinePointClick(index)}ck={() => handleTimelinePointClick(index)}
                    >
                        <div 
                            className={`
                                ${styles.timelinePoint}   ${styles.timelinePoint} 
                                ${activeMonth === index ? styles.active : ''}      ${activeMonth === index ? styles.active : ''}
                            `}
                        />
                        <div className={`
                            ${styles.timelineMonth} ${styles.timelineMonth}
                            ${activeMonth === index ? styles.current : ''}eMonth === index ? styles.current : ''}
                        `}>
                            {month}  {month}
                        </div>     </div>
                    </div>  </div>
            ...
            <svg style={{ position: 'absolute', width: 0, height: 0 }}>lute', width: 0, height: 0 }}>
                <defs>
                    <filter id="texture">
                        <feTurbulence 
                            type="fractalNoise" ise" 
                            baseFrequency="0.65" uency="0.65" 
                            numOctaves="3" 
                            seed="1"  seed="1"
                            stitchTiles="stitch"itch"
                        />
                        <feDisplacementMap ntMap 
            ...
            {activeQuestionData && (
                <div className={`${styles.overlay} ${isZooming ? styles.visible : ''}`}>{`${styles.overlay} ${isZooming ? styles.visible : ''}`}>
                    <div className={`${styles.contentWrapper} ${isZooming ? styles.entering : styles.leaving}`}>per} ${isZooming ? styles.entering : styles.leaving}`}>
                        <button
                            className={styles.backButton}   className={styles.backButton}
                            onClick={handleBack}
                        >
                            <ArrowLeftIcon className="mr-2" size={20} />wLeftIcon className="mr-2" size={20} />
                            <span>Back to Questions</span>                            <span>Back to Questions</span>
                        </button>tton>

                        <div
                            className={styles.activeQuestion}
                            style={{
                                backgroundColor: activeQuestionData.color,  backgroundColor: activeQuestionData.color,
                                boxShadow: `0 0 100px ${activeQuestionData.color}, inset 0 0 165px ${activeQuestionData.color}`       boxShadow: `0 0 100px ${activeQuestionData.color}, inset 0 0 165px ${activeQuestionData.color}`
                            }}
                        >
                            <div className={styles.questionText}>lassName={styles.questionText}>
                                {activeQuestionData.question[language]}  {activeQuestionData.question[language]}
                            </div>                            </div>
                        </div>

                        <div className={styles.answersContainer}>me={styles.answersContainer}>
                            {currentAnswers.map((answer, index, array) => ((
                                <div
                                    key={`answer-${index}-${currentPage}`}swer-${index}-${currentPage}`}
                                    className={styles.answerBox}
                                    style={{
                                        backgroundColor: activeQuestionData.color,  backgroundColor: activeQuestionData.color,
                                        '--hover-color': activeQuestionData.color,       '--hover-color': activeQuestionData.color,
                                    }}
                                >
                                    {answer}     {answer}
                                </div>  </div>
